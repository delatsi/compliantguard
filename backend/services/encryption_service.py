...
class CustomerEncryptionService:
    ...
    async def _create_customer_key(self) -> str:
        ...
        self.kms.create_alias(AliasName=self.customer_key_alias, TargetKeyId=key_id)
    ...
    async def encrypt_customer_data(
        ...
        dek_response = self.kms.generate_data_key(
            KeyId=key_id, KeySpec="AES_256", EncryptionContext=required_context
        )
    ...
    async def decrypt_customer_data(
        self, encrypted_record: Dict[str, Any], context: Dict[str, str]
    ) -> Dict[str, Any]:
        """
        ...
        if encrypted_record["customer_id"] != self.customer_id:
            raise PermissionError(
                f"Cannot decrypt data for different customer: {encrypted_record['customer_id']}"
            )
    ...
    async def encrypt_field(self, field_value: str, field_name: str) -> Dict[str, str]:
        ...
        encrypted_data = await self.encrypt_customer_data(
            {"field_value": field_value}, context
        )
        ...
        return {
            ...
            "field_hash": hashlib.sha256(field_value.encode("utf-8")).hexdigest()[
                :16
            ],  # For searching
    ...
    async def rotate_customer_key(self) -> str:
        ...
        self.kms.update_alias(AliasName=self.customer_key_alias, TargetKeyId=new_key_id)
