...
class GCPCredentialService:
    ...
    def __init__(self):
        ...
        gcp_table_name = getattr(
            settings,
            "GCP_CREDENTIALS_TABLE",
            settings.DYNAMODB_TABLE_NAME.replace("-scans", "-gcp-credentials"),
        )
        self.creds_table = self.dynamodb.Table(gcp_table_name)
        self.kms_key_alias = getattr(
            settings, "KMS_KEY_ALIAS", "alias/compliantguard-gcp-credentials"
        )

    async def store_credentials(
        self, user_id: str, project_id: str, service_account_json: Dict
    ) -> Dict:
        """
        ...
        try:
            ...
            try:
                encrypted_response = self.kms_client.encrypt(
                    KeyId=self.kms_key_alias, Plaintext=credentials_str.encode("utf-8")
                )
            except ClientError as e:
                logger.error("kms_encryption_failed", error=str(e))
                raise HTTPException(
                    status_code=500, detail="Failed to encrypt credentials"
                )
            ...
            logger.info(
                "gcp_credentials_stored_successfully",
                user_id=user_id,
                project_id=project_id,
                credential_id=credential_id,
            )
            ...
            raise HTTPException(
                status_code=500, detail="Failed to store GCP credentials"
            )
    ...
    async def get_credentials(self, user_id: str, project_id: str) -> Dict:
        ...
        try:
            logger.info(
                "retrieving_gcp_credentials", user_id=user_id, project_id=project_id
            )
            ...
            if "Item" not in response:
                logger.warning(
                    "credentials_not_found", user_id=user_id, project_id=project_id
                )
                raise HTTPException(
                    status_code=404, detail="GCP credentials not found for this project"
                )
            ...
            if item.get("status") != "active":
                ...
                raise HTTPException(
                    status_code=403, detail="GCP credentials are not active"
                )
            ...
            logger.info(
                "gcp_credentials_retrieved_successfully",
                user_id=user_id,
                project_id=project_id,
            )
            ...
            raise HTTPException(
                status_code=500, detail="Failed to retrieve GCP credentials"
            )
    ...
    async def revoke_credentials(self, user_id: str, project_id: str) -> Dict:
        ...
        try:
            logger.info(
                "revoking_gcp_credentials", user_id=user_id, project_id=project_id
            )
            ...
            logger.info(
                "gcp_credentials_revoked", user_id=user_id, project_id=project_id
            )
    ...
    async def _validate_service_account(self, creds: Dict, project_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "service_account_validation_successful",
                    project_id=project_id,
                ...
                if e.resp.status == 403:
                    ...
                    raise HTTPException(
                        status_code=400, detail=f"GCP API error: {e.resp.reason}"
                    )
            ...
            logger.error(
                "service_account_validation_failed", project_id=project_id, error=str(e)
            )
            raise HTTPException(
                status_code=400, detail="Invalid service account credentials"
            )
