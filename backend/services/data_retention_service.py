...
class DataRetentionService:
    ...
    async def delete_customer_data_by_request(
        ...
        try:
            ...
            for item in customer_items:
                try:
                    ...
                    if success:
                        ...
                        deletion_summary['errors'].append(f"Failed to delete {item['resource_type']}:{item['resource_id']}")
                        
                except Exception as e:
                    deletion_summary['errors'].append(f"Error processing {item.get('resource_id', 'unknown')}: {str(e)}")
            ...
            try:
                ...
                deletion_summary['errors'].append(f"Failed to delete encryption keys: {str(e)}")
            ...
            await self.audit_service.log_access(AuditEvent(
                ...
                result=AccessResult.SUCCESS if not deletion_summary['errors'] else AccessResult.PARTIAL,
                timestamp=datetime.utcnow(),
    ...
    async def _process_automatic_deletion(self, item: Dict[str, Any]) -> bool:
        ...
        try:
            deletion_method = DeletionMethod(item['deletion_method'])
            return await self._execute_deletion(item, deletion_method, reason="automatic_retention_policy")
        except Exception as e:
    ...
    async def _execute_deletion(
        ...
        try:
            ...
            if deletion_method == DeletionMethod.CRYPTOGRAPHIC_ERASURE:
                ...
                await self._hard_delete_resource(customer_id, resource_type, resource_id)
                
            elif deletion_method == DeletionMethod.SOFT_DELETE:
                # Mark as deleted but keep encrypted
                await self._soft_delete_resource(customer_id, resource_type, resource_id)
    ...
    async def _hard_delete_resource(self, customer_id: str, resource_type: str, resource_id: str):
    ...
    async def _soft_delete_resource(self, customer_id: str, resource_type: str, resource_id: str):
