...
from ..models.compliance import ComplianceReport, Violation, ViolationSeverity, ViolationType, DashboardData, ViolationSummary
...
class ComplianceService:
            ...
            for policy in policy_evaluations:
                try:
                    result = subprocess.run([
                        'opa', 'eval',
                        '--input', temp_assets_file,
                        '--data', '/Users/delatsi/projects/themisguard/policies',
                        '--format', 'json',
                        policy["query"]
                    ], capture_output=True, text=True, check=True)
                    
                    policy_violations = json.loads(result.stdout)['result'][0]['expressions'][0]['value']
                    
                    for i, violation_text in enumerate(policy_violations):
                        violation = self._parse_violation(violation_text, policy["type"], i)
    ...
    def _parse_violation(self, violation_text: str, violation_type: ViolationType, index: int) -> Violation:
        ...
        elif "High" in violation_text or "unrestricted access" in violation_text or "Violation (High)" in violation_text:
            severity = ViolationSeverity.HIGH
        elif "Violation (Medium)" in violation_text or "Environment Separation Violation (Medium)" in violation_text:
            severity = ViolationSeverity.MEDIUM
        elif "Review Required" in violation_text or "Violation (Low)" in violation_text or "Warning" in violation_text:
        ...
        return Violation(
            ...
            type=violation_type,
            severity=severity,
            title=violation_text.split(" - ")[0] if " - " in violation_text else violation_text[:50],
    ...
    def _get_remediation_steps(self, violation_text: str) -> List[str]:
        ...
        elif "Firewall rule" in violation_text and "unrestricted access" in violation_text:
            steps = [
        ...
        elif "Environment Separation" in violation_text and "access controls" in violation_text:
            steps = [
    ...
    async def store_scan_results(self, user_id: str, project_id: str, violations: List[Violation]) -> str:
        """Store scan results in DynamoDB and S3"""
        scan_id = str(uuid.uuid4())
        
        # Calculate compliance metrics
        violation_counts = {
            ViolationSeverity.CRITICAL: sum(1 for v in violations if v.severity == ViolationSeverity.CRITICAL),
            ViolationSeverity.HIGH: sum(1 for v in violations if v.severity == ViolationSeverity.HIGH),
            ViolationSeverity.MEDIUM: sum(1 for v in violations if v.severity == ViolationSeverity.MEDIUM),
            ViolationSeverity.LOW: sum(1 for v in violations if v.severity == ViolationSeverity.LOW)
        }
        
        # Calculate compliance score (simple formula)
        total_violations = len(violations)
        compliance_score = max(0, 100 - (violation_counts[ViolationSeverity.CRITICAL] * 20 + 
    ...
    async def list_user_reports(self, user_id: str, limit: int = 10, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    async def get_dashboard_summary(self, user_id: str) -> DashboardData:
        ...
        try:
            ...
            if scans:
                overall_compliance_score = sum(scan.get('compliance_score', 0) for scan in scans) / len(scans)
