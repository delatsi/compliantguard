...
class AuditService:
        ...
        try:
            ...
            if event.phi_accessed and not event.minimum_necessary:
                await self._flag_compliance_violation(
                    event, "PHI accessed without minimum necessary check"
                )

            if event.approval_required and not event.approved_by:
                await self._flag_compliance_violation(
                    event, "Required approval not obtained"
                )
    ...
    async def detect_anomalous_patterns(
        ...
        try:
            ...
            recent_events = await self.get_audit_trail(
                user_id=user_id, start_time=start_time, end_time=end_time, limit=1000
            )
            ...
            failed_logins = [
                ...
                and e.get("result") == AccessResult.DENIED.value
            ]
            ...
            unique_ips = set(
                e.get("ip_address") for e in recent_events if e.get("ip_address")
            )
            if len(unique_ips) > 3:  # Multiple IPs in short time
                pattern = DataAccessPattern(
                    user_id=user_id,
                    customer_id=(
                        recent_events[0].get("customer_id") if recent_events else None
                    ),
            ...
            if total_requests > baseline_requests * 3:
                pattern = DataAccessPattern(
                    ...
                    deviation_score=(total_requests - baseline_requests)
                    / baseline_requests,
                    timestamp=datetime.utcnow(),
    ...
    async def generate_compliance_report(
        ...
        try:
            ...
            phi_accesses = len(
                [e for e in all_events if e.get("data_classification") == "PHI"]
            )
            unauthorized_attempts = len(
                [e for e in all_events if e.get("result") == AccessResult.DENIED.value]
            )
            unique_users = len(
                set(e.get("user_id") for e in all_events if e.get("user_id"))
            )
            ...
            report = {
                ...
                "phi_access_details": [
                    ...
                    for e in all_events
                    if e.get("data_classification") == "PHI"
                ],
    ...
    async def _trigger_pattern_alert(self, pattern: DataAccessPattern):
        ...
        await self._create_security_event(
            (
                ...
                if "user_agent" in pattern.pattern_type
                else SecurityEventType.UNUSUAL_IP_ACCESS
            ),
            Severity.MEDIUM,
    ...
    async def _count_recent_failures(self, user_id: str, minutes: int) -> int:
        ...
        return len(
            [e for e in recent_events if e.get("result") == AccessResult.DENIED.value]
        )
    ...
    async def _flag_compliance_violation(
        self, event: ComplianceEvent, violation_description: str
    ):
