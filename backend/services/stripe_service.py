...
class StripeService:
    ...
    async def create_customer(
        self, user_id: str, email: str, name: str, company: Optional[str] = None
    ) -> str:
        ...
        try:
            customer_data = {
                "email": email,
                "name": name,
                "metadata": {"user_id": user_id, "company": "DELATSI LLC"},  # Your LLC
            }
    ...
    async def create_subscription(
        ...
        try:
            ...
            if not price_id:
                raise Exception(
                    f"Stripe price ID not configured for {plan_tier} {billing_interval}"
                )
    ...
    async def change_subscription(
        self, request: SubscriptionChangeRequest
    ) -> CustomerSubscription:
        """Change customer subscription plan or billing interval"""
        try:
            current_subscription = await self.get_customer_subscription(
                request.customer_id
            )
            ...
            price_id = (
                ...
                if request.new_billing_interval == BillingInterval.ANNUAL
    ...
    async def cancel_subscription(
        self, user_id: str, cancel_immediately: bool = False
    ) -> CustomerSubscription:
    ...
    async def record_usage(
        self, user_id: str, scans_count: int, projects_scanned: int, api_calls: int = 0
    ):
    ...
    async def get_monthly_usage(
        self, user_id: str, month_year: Optional[str] = None
    ) -> Dict[str, int]:
        ...
        try:
            ...
            # Aggregate usage
            total_scans = sum(
                item.get("scans_count", 0) for item in response.get("Items", [])
            )
            total_projects = len(
                set(
                    item.get("projects_scanned", [])
                    for item in response.get("Items", [])
                )
            )
            total_api_calls = sum(
                item.get("api_calls", 0) for item in response.get("Items", [])
            )
