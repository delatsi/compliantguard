...
class CustomerDataService:
    ...
    def __init__(self, customer_id: str, user_id: str, user_role: Role, request_context: Dict[str, Any] = None):
    ...
    async def _audit_access(self, action: str, resource_type: str, resource_id: str = None, 
                           result: AccessResult = AccessResult.SUCCESS, error: str = None):
        """Audit all data access attempts"""
        await self.audit_service.log_access(AuditEvent(
    ...
    async def store_scan_data(self, scan_data: Dict[str, Any]) -> str:
        ...
        if not self._has_permission(Permission.WRITE_CUSTOMER_DATA):
            await self._audit_access("store_scan_data", "scan_data", result=AccessResult.DENIED)
        ...
        try:
            ...
            await self._audit_access("store_scan_data", "scan_data", scan_id, AccessResult.SUCCESS)
            
            return scan_id
            
        except Exception as e:
            await self._audit_access("store_scan_data", "scan_data", result=AccessResult.ERROR, error=str(e))
            raise
    ...
    async def get_scan_data(self, scan_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not self._has_permission(Permission.READ_CUSTOMER_DATA):
            await self._audit_access("get_scan_data", "scan_data", scan_id, AccessResult.DENIED)
        ...
        try:
            ...
            if 'Item' not in response:
                await self._audit_access("get_scan_data", "scan_data", scan_id, AccessResult.NOT_FOUND)
                return None
            ...
            await self._audit_access("get_scan_data", "scan_data", scan_id, AccessResult.SUCCESS)
            
            return decrypted_data
            
        except Exception as e:
            await self._audit_access("get_scan_data", "scan_data", scan_id, AccessResult.ERROR, error=str(e))
            raise

    async def list_customer_scans(self, limit: int = 50, start_key: str = None) -> Dict[str, Any]:
        """List scans for customer with pagination"""
        
        # Check permissions
        if not self._has_permission(Permission.READ_CUSTOMER_DATA):
            await self._audit_access("list_customer_scans", "scan_data", result=AccessResult.DENIED)
        ...
        try:
            ...
            for item in response.get('Items', []):
                try:
                    decrypted_item = await self.encryption_service.decrypt_customer_data(
                        encrypted_data=item,
                        context={
                            'customer_id': self.customer_id,
                    ...
                    print(f"Failed to decrypt item {item.get('scan_id', 'unknown')}: {e}")
                    continue
            ...
            # Audit successful access
            await self._audit_access("list_customer_scans", "scan_data", result=AccessResult.SUCCESS)
            
            return result
            
        except Exception as e:
            await self._audit_access("list_customer_scans", "scan_data", result=AccessResult.ERROR, error=str(e))
            raise
    ...
    async def delete_scan_data(self, scan_id: str) -> bool:
        ...
        if not self._has_permission(Permission.DELETE_CUSTOMER_DATA):
            await self._audit_access("delete_scan_data", "scan_data", scan_id, AccessResult.DENIED)
        ...
        try:
            ...
            if not existing_item:
                await self._audit_access("delete_scan_data", "scan_data", scan_id, AccessResult.NOT_FOUND)
                return False
            
            if existing_item.get('customer_id') != self.customer_id:
                await self._audit_access("delete_scan_data", "scan_data", scan_id, AccessResult.DENIED)
                raise PermissionError("Cannot delete scan data belonging to different customer")
            ...
            await self._audit_access("delete_scan_data", "scan_data", scan_id, AccessResult.SUCCESS)
            
            return True
            
        except Exception as e:
            await self._audit_access("delete_scan_data", "scan_data", scan_id, AccessResult.ERROR, error=str(e))
            raise
    ...
    async def export_customer_data(self, export_format: str = 'json') -> str:
        ...
        if not self._has_permission(Permission.EXPORT_CUSTOMER_DATA):
            await self._audit_access("export_customer_data", "customer_data", result=AccessResult.DENIED)
        ...
        try:
            ...
            all_scans = await self.list_customer_scans(limit=1000)  # Adjust based on needs
            ...
            export_key = f"{self.s3_prefix}exports/{export_id}.{export_format}.encrypted"
            ...
            # Audit the export
            await self._audit_access("export_customer_data", "customer_data", export_id, AccessResult.SUCCESS)
            
            return download_url
            
        except Exception as e:
            await self._audit_access("export_customer_data", "customer_data", result=AccessResult.ERROR, error=str(e))
            raise
    ...
    async def purge_customer_data(self) -> Dict[str, int]:
        ...
        if not self._has_permission(Permission.DELETE_CUSTOMER_DATA):
            await self._audit_access("purge_customer_data", "customer_data", result=AccessResult.DENIED)
        ...
        try:
            ...
            await self._audit_access("purge_customer_data", "customer_data", result=AccessResult.SUCCESS)
            
            return deletion_counts
            
        except Exception as e:
            await self._audit_access("purge_customer_data", "customer_data", result=AccessResult.ERROR, error=str(e))
            raise
    ...
    def decorator(func):
        async def wrapper(self, *args, **kwargs):
            ...
            if not isinstance(self, CustomerDataService):
                raise TypeError("secure_customer_access can only be used on CustomerDataService methods")
            ...
            if not self._has_permission(permission):
                ...
                raise PermissionError(f"Permission {permission.value} required for {func.__name__}")
