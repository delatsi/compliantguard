...
def handler(event, context):
    ...
    try:
        ...
        if path == '/health':
            ...
            print(f"📝 Service Account Keys: {list(service_account_json.keys()) if service_account_json else 'None'}")
            
            if not project_id or not service_account_json:
                response_body = {"error": "Missing required fields: project_id, service_account_json"}
            else:
                try:
                    ...
                    print(f"✅ GCP credentials would be stored for project: {project_id}")
                    ...
                    response_body = {"error": f"Failed to store GCP credentials: {str(e)}"}
