...
async def debug_info():
    ...
    debug_info = {
        ...
        "aws_connectivity": {},
        "tables": {},
        "versions": {"python": sys.version, "boto3": boto3.__version__},
    }
        ...
        if report:
            ...
            if isinstance(report, dict):
                if "violations" in report:
                    violations_count = (
                        len(report["violations"]) if report["violations"] else 0
                    )
...
async def list_reports(
    ...
    try:
        ...
        if reports and len(reports) > 0:
            print(
                f"📊 First report keys: {list(reports[0].keys()) if isinstance(reports[0], dict) else 'Not a dict'}"
            )
...
async def get_admin_dashboard(admin_user=Depends(verify_admin_token)):
    ...
    try:
        ...
        dashboard_data = await admin_service.get_admin_dashboard_data(
            admin_user.admin_id
        )
...
async def get_admin_audit_logs(admin_user=Depends(verify_admin_token), limit: int = 50):
...
async def create_stripe_customer(current_user: dict = Depends(get_current_user)):
...
async def get_user_subscription(current_user: dict = Depends(get_current_user)):
    ...
    try:
        subscription = await stripe_service.get_customer_subscription(
            current_user["user_id"]
        )
        if not subscription:
...
async def change_subscription(
    request: SubscriptionChangeRequest, current_user: dict = Depends(get_current_user)
):
...
async def cancel_subscription(
    cancel_immediately: bool = False, current_user: dict = Depends(get_current_user)
):
    """Cancel user's subscription"""
    try:
        subscription = await stripe_service.cancel_subscription(
            current_user["user_id"], cancel_immediately
        )
        return subscription
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_billing_usage(
    month_year: Optional[str] = None, current_user: dict = Depends(get_current_user)
):
    """Get user's usage for billing"""
    try:
        usage = await stripe_service.get_monthly_usage(
            current_user["user_id"], month_year
        )
        limits_check = await stripe_service.check_usage_limits(current_user["user_id"])

        return {"usage": usage, "limits_check": limits_check}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_billing_portal(
    return_url: str, current_user: dict = Depends(get_current_user)
):
    """Create Stripe customer portal session"""
    try:
        portal_url = await stripe_service.create_billing_portal_session(
            current_user["user_id"], return_url
        )
...
async def record_usage(
    ...
    try:
        await stripe_service.record_usage(
            current_user["user_id"], scans_count, projects_scanned, api_calls
        )
