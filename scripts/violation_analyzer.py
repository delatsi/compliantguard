...
def get_raw_violations():
    ...
    try:
        ...
        themisguard_violations = json.loads(result.stdout)['result'][0]['expressions'][0]['value']
...
def extract_violation_pattern(violation_str):
    ...
    if 'private' in violation_str and ('cluster' in violation_str or 'gke' in violation_str):
    ...
    elif 'storage' in violation_str and ('bucket' in violation_str or 'public' in violation_str):
...
def extract_violation_type(violation_str):
    ...
    if 'private' in violation_str and 'cluster' in violation_str:
        ...
        return 'Service Account Issues'
    elif 'public' in violation_str and ('ip' in violation_str or 'access' in violation_str):
...
"""
    ...
    for vtype, count in list(analysis['violation_types'].items())[:10]:
        ...
        report += f"- **{vtype}**: {count} instances ({percentage:.1f}%) - *{severity}*\n"
...
"""
    ...
    for i, item in enumerate(analysis['raw_violations'][:5], 1):
        ...
        if isinstance(violation, dict):
            title = violation.get('title', violation.get('message', violation.get('description', 'Unknown')))
            resource = violation.get('resource', violation.get('asset', 'Unknown'))
        else:
            title = str(violation)[:100] + "..." if len(str(violation)) > 100 else str(violation)
            resource = "Unknown"
