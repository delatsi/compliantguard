...
class ProductRenamer:
    def __init__(self, old_name: str, new_name: str, dry_run: bool = False):
        ...
        self.backup_dir = self.project_root / "backups" / f"rename-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        ...
        for pattern in self.skip_patterns:
            if file_path.match(pattern) or str(rel_path).startswith(pattern.rstrip('/*')):
                return True
    ...
    def _create_backup(self):
        ...
        for item in self.project_root.iterdir():
            if item.name not in {'.git', 'node_modules', '__pycache__', '.aws-sam', 'backups'}:
                if item.is_dir():
    ...
    def _find_files_to_process(self) -> List[Path]:
        ...
        for file_path in self.project_root.rglob('*'):
            if file_path.is_file() and not self._should_skip_file(file_path):
                if self._is_text_file(file_path) or self._contains_old_name_in_filename(file_path):
        ...
        try:
            ...
            for old_text, new_text in self.replacements.items():
                if old_text in content:
                    ...
                    if not self.dry_run:
                        print(f"  - Replaced '{old_text}' → '{new_text}' ({replacements_count} times)")
    ...
    def _update_package_json(self, package_json_path: Path):
        ...
        try:
            ...
            if 'name' in data and self.old_name.lower() in data['name']:
                old_name = data['name']
                data['name'] = data['name'].replace(self.old_name.lower(), self.new_name.lower())
    ...
    def _update_sam_config(self, sam_config_path: Path):
        ...
        try:
            ...
            if old_stack_pattern in content:
                ...
                print(f"📋 Updated SAM stack names: {old_stack_pattern} → {new_stack_pattern}")
    ...
    def run(self):
        ...
        print(f"🔄 {'DRY RUN: ' if self.dry_run else ''}Renaming product from '{self.old_name}' to '{self.new_name}'")
...
def main():
    parser = argparse.ArgumentParser(description="Rename product across entire codebase")
    parser.add_argument("--old-name", required=True, help="Current product name (e.g., 'ThemisGuard')")
    parser.add_argument("--new-name", required=True, help="New product name (e.g., 'NewProductName')")
    ...
    if not re.match(r'^[a-zA-Z][a-zA-Z0-9]*$', args.new_name):
        print("❌ Error: New name should start with a letter and contain only alphanumeric characters")
        return
