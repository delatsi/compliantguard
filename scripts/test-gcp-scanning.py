...
def login_and_get_token(creds):
    ...
    try:
        response = requests.post(
            f"{creds['api_url']}/api/v1/auth/login", json=login_data
        )
...
def upload_gcp_credentials(api_url, token, gcp_creds_file):
    ...
    try:
        ...
        response = requests.post(
            f"{api_url}/api/v1/gcp/credentials", json=upload_data, headers=headers
        )
...
def trigger_compliance_scan(api_url, token, project_id):
    ...
    try:
        ...
        response = requests.post(
            f"{api_url}/api/v1/scan", json=scan_data, headers=headers
        )
