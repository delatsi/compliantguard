    ...
    try:
        ...
        for api, service_name in firebase_api_mapping.items():
            if api in enabled_apis:
                firebase_services[service_name] = {
                    'api': api,
                    'enabled': True,
                    'hipaa_relevant': True if any(keyword in service_name.lower()
                                                for keyword in ['firestore', 'database', 'storage', 'auth', 'rules']) else False
                }
        ...
        if firebase_services:
            ...
            if 'firebaserules.googleapis.com' in enabled_apis:
                print("✅ firebaserules.googleapis.com IS in the list - this should have been detected!")
            else:
...
def scan_firestore_compliance(project_id):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("⚠️ Could not access Firestore databases (may not exist or access denied)")
...
def scan_firebase_storage_compliance(project_id):
    ...
    try:
        ...
        for bucket in buckets:
            ...
            if bucket.get('iamConfiguration', {}).get('uniformBucketLevelAccess', {}).get('enabled') != True:
                violations.append({
...
def generate_firebase_hipaa_report(project_id):
    ...
    print(f"   Violations: {len(all_violations)} (Critical: {critical_count}, High: {high_count}, Medium: {medium_count}, Low: {low_count})")
    ...
    for violation in firebase_report['violations']:
        formatted_violation = {
            ...
            'remediation': {
                ...
                'effort': 'Medium',
                'cost': '$1,000 - $5,000',
                'timeline': '1-2 weeks' if violation['severity'] in ['HIGH', 'CRITICAL'] else '2-4 weeks',
                'priority': violation['severity'].title()
            }
        }

        if 'manual_check' in violation:
            formatted_violation['remediation']['manual_check'] = violation['manual_check']
            formatted_violation['remediation']['action'] += f" (Manual review required: {violation['manual_check']})"
