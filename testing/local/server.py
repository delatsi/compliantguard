...
class TestServerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            user_projects = [
                p for p in gcp_projects_db.values() if p["user_id"] == user["user_id"]
            ]
    ...
    def do_POST(self):
        ...
        if path == "/api/v1/auth/register":
            ...
            if existing:
                self.send_json_response(
                    {"detail": "User with this email already exists"}, 400
                )
                return
            ...
            if not project_id or not service_account_json:
                self.send_json_response(
                    {"detail": "project_id and service_account_json required"}, 400
                )
                return
            ...
            if service_account_json.get("type") != "service_account":
                self.send_json_response(
                    {"detail": "Must be a service account key"}, 400
                )
                return
