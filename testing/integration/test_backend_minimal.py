    ...
    try:
        ...
        if creds_table:
            item = {
                ...
                "service_account_email": credential_data.service_account_json[
                    "client_email"
                ],
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to store GCP credentials: {str(e)}"
        )
...
async def upload_gcp_credentials_file(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to upload GCP credentials: {str(e)}"
        )
...
async def list_gcp_projects(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list GCP projects: {str(e)}"
        )
...
async def revoke_gcp_credentials(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to revoke GCP credentials: {str(e)}"
        )
...
async def validate_service_account_structure():
    ...
    return {
        ...
        and sample_sa.get("type") == "service_account",
    }
